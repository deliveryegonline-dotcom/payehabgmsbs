...
export const etisalatCashAdapter: WalletAdapter = {
  id: 'etisalat_cash',
  name: 'Etisalat Cash (e& cash)',
  arabicName: 'إي آند كاش (اتصالات كاش)',
  senders: ['EtisalatCash', 'e&cash', 'Etisalat-Cash', 'eandcash', 'etisalat'],

  canHandle(sender: string, rawText: string): boolean {
    const s = sender.toLowerCase();
    return (
      this.senders.some((name) => s.includes(name.toLowerCase())) ||
      rawText.includes('اتصالات كاش') ||
      rawText.includes('e& cash')
    );
  },
